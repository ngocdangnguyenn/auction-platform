...
from wtforms import DecimalField, StringField, IntegerField, SelectField, SubmitField, ValidationError
...
class MonCompteForm(FlaskForm):
    ...
    pays = StringField('Pays', validators=[DataRequired(), Length(max=100)])
...
class AcheterJetonsForm(FlaskForm):
    ...
    pack_jetons = SelectField('Pack de jetons', coerce=int, validators=[DataRequired()])
...
class PlacerEnchereForm(FlaskForm):
    montant = DecimalField('Montant de l\'enchère (€)', validators=[DataRequired(), NumberRange(min=0.01)])
    ...
    def validate_montant(self, field):
        ...
        if enchere and field.data <= enchere.prix_depart:
            raise ValidationError(f"Le montant doit être supérieur au prix de départ ({enchere.prix_depart} €).")
